    ...
    <section class="content">
        ...
        <div id="accordion">
          ...
          ?>
            ...
            <?php } else { ?>
                <div class="col-lg-3">
                  <a class="d-block w-100" <?php $viewedsql = "SELECT * FROM lessonsviewed WHERE userId = '".$_SESSION['id']."' AND chapterId = '".$row['chapter_id']."' AND lessonId = '".$_GET['id']."' AND viewed = 'viewed'"; $viewedstmt = $pdo -> prepare($viewedsql); $viewedstmt->execute(); $viewedrow = $viewedstmt->fetch(); if($viewedrow){ echo '';} else {?>onclick="return confirm('Are you sure you want to start? Once started all your inputs will be recorded. If you try to exit the test all of the left questions will mark as 0 score, so be careful of your actions.')"<?php } ?>  href="<?php $viewedsql = "SELECT * FROM lessonsviewed WHERE userId = '".$_SESSION['id']."' AND chapterId = '".$row['chapter_id']."' AND lessonId = '".$_GET['id']."' AND viewed = 'viewed'"; $viewedstmt = $pdo -> prepare($viewedsql); $viewedstmt->execute(); $viewedrow = $viewedstmt->fetch(); if($viewedrow){ echo 'quiz-score.php?qid='.$row['chapter_id'] .'&lid='.$_GET['id'];} else { ?>quiz-view.php?qid=<?php echo $row['chapter_id'];?>&lid=<?php echo $_GET['id'];?>&id=<?php $sql = "SELECT * FROM questions WHERE quiz_id = '".$row['chapter_id']."' ORDER BY question_id ASC"; $stmt = $pdo -> prepare($sql);  $stmt->execute(); $getQId = $stmt->fetch(); echo $getQId['question_id']; ?><?php } ?>">
                  <div class="card <?php $viewedsql = "SELECT * FROM lessonsviewed WHERE userId = '".$_SESSION['id']."' AND chapterId = '".$row['chapter_id']."' AND lessonId = '".$_GET['id']."' AND viewed = 'viewed'"; $viewedstmt = $pdo -> prepare($viewedsql); $viewedstmt->execute(); $viewedrow = $viewedstmt->fetch(); if($viewedrow){ echo 'bg-gradient-success';} else { echo 'bg-gradient-secondary';} ?> card-outline">
